...
while (true) {
    ...
    //if (move) pathIsSafe(map, move.x, move.y, true); //print why this move is safe
    ...
    if (move!=null && move.run && canDestroySomething() && REMAINING_BOMBS>0) {
...
function moveAfterBomb(bomb) {
    ...
    //printErr('if we all place bombs... ');
    //printSafetyMap(newMap);

    //if (move != null) printErr('safe move found if we place a bomb: ' + JSON.stringify(move));
...
function minTurnsLeftOfBombsInRange(map, bombX, bombY) {
    ...
    //printErr('bomb ' + bombX + ',' + bombY + ' has ' + bombs.length + ' bombs in range. min=' + min);
    return min;
}
...
function pathIsSafe(map, x, y, print) {
    //if (map[x][y].steps == 0) return true;
